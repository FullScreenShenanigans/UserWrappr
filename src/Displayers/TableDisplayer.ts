import { ISizeSummary } from "../SizeChanger";
import { IChoiceElement, IInputElement, IOption, ISchema, ISelectElement } from "../UISchemas";
import { OptionsDisplayer } from "./OptionsDisplayer";
 ...
 */
export class TableDisplayer extends OptionsDisplayer<IOptionsTableSchema> {
    /**
    ...
    protected setKeyInput(input: IInputElement, details: IOptionsTableKeysOption): ISelectElement[] {
        ...
        for (const value of values) {
            ...
            for (let j: number = 0; j < OptionsDisplayer.allPossibleKeys.length; j += 1) {
                child.appendChild(new Option(OptionsDisplayer.allPossibleKeys[j]));

                // Setting child.value won't work in IE or Edge...
                if (OptionsDisplayer.allPossibleKeys[j] === valueLower) {
    ...
    protected setScreenSizeInput(input: ISelectElement, details: IOptionsTableScreenSizeOption): ISelectElement {
        details.options = (): string[] => Object.keys(this.settings.getSizes());

        details.source = (): string => this.settings.getSize().name!;

        details.update = (value: ISizeSummary | string): void => {
            if (value !== this.settings.getSize()) {
    ...
    protected ensureLocalStorageInputValue(childRaw: IChoiceElement | IChoiceElement[], details: IOption, schema: ISchema): void {
        ...
        this.settings.gameStarter.itemsHolder.addItem(key, {
            "storeLocally": true,
            "valueDefault": valueDefault
        });

        const value: string = this.settings.gameStarter.itemsHolder.getItem(key);
    ...
    protected storeLocalStorageValue(child: IInputElement | ISelectElement, value: any): void {
        ...
        if (key) {
            this.settings.gameStarter.itemsHolder.setItem(key, value);
            this.settings.gameStarter.itemsHolder.saveItem(key);
        }
    }
    ...
    protected getOptionType(typeName: string):
        ...
        switch (typeName) {
            case "Boolean":
                return TableDisplayer.prototype.setBooleanInput;
            case "Keys":
                return TableDisplayer.prototype.setKeyInput;
            case "Number":
                return TableDisplayer.prototype.setNumberInput;
            case "Select":
                return TableDisplayer.prototype.setSelectInput;
            case "ScreenSize":
                return TableDisplayer.prototype.setScreenSizeInput;
            default:
